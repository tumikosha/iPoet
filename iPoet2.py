...
if __name__ == "__main__":
    print("::",numSlogs("привет"))
    phrase=u"Я - Аборди́рованный  зовусь я Миша. От меня вам фунт гашиша!"
    phrase="  СКИДКИ ДО КОНЦА МЕСЯЦА "
    list=stringTokenizer(phrase,", !- ")
    print(list)
    #wordDict=loadWordDict("udar.txt")
    word="Аборди́рованный"
    #word="Аба́з"
    #word="Аба́з"
    print(":3:",slogize3(word),slogTokenizer3(word))
    print(":0:",slogize(word),slogTokenizer(word),)

    word="Абаз"
    #print(":1>",detectUdar(word))
    print(":3:",slogize3(word))
    print(":0:",slogize(word))
    print("Scheme:"+wordScheme(word))
    scheme,list1,list2=sentenceScheme(phrase)
    print("sentenceScheme:"+scheme,list1,list2)
    print(phrase)
    print("num:"+str(wordNumByPositionInScheme(list1,3)))
    print(filterPhrase(phrase,"u u-u"))
    print("-----------------------------")
    txt="Привет!   Давай   знакомиться ?! Ты такая   красивая, я не смог пройти мимо! ;))"
    parser=Parser()
    parser.parse(txt)
    print("# ", parser.sentenceList)
